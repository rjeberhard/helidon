...
* token-bucket: Permits start at the configured amount and refill up to that amount over the duration. This is the default algorithm.
* fixed-rate: There is only ever one permit and, if consumed, a new permit is generated at the necessary rate to achieve the configured throughput. This algorithm is also called "leaky bucket". It is very important to configure queuing as this algorithm almost always has a backlog.

Here is the same example showing the configuration of the fixed-rate rate limiting algorithm:
[source,yaml]
----
server:
  concurrency-limit:
    throughput:
      amount: 1000
      duration: PT5S
      rate-limiting-algorithm: fixed-rate
      queue-length: 200
      queue-timeout: PT1S
----
